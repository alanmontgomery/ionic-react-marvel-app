...
const Home = () => {
	...
	useEffect(() => {
		...
		const getCharacters = async () => {
			...
			const response = await fetch("http://importmarvel.com/api/characters?limit=20&orderBy=-modified");
	...
	const fetchMore = async (e) => {
		...
		const response = await fetch(`http://importmarvel.com/api/characters?limit=20&offset=${ amountLoaded }&orderBy=-modified`);
		const data = await response.json();
		const results = data.data.results;
